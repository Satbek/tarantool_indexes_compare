...
local function generate_pks(count)
    for i = 1, count do
        pks[i] = uuid.str()
    end
end

local function generate_secondaries(count)
    for i = 1, count do
        secondaries[i] = uuid.str()
    end
end
...
generate_pks(count)
generate_secondaries(count)
generate_replace_ammo(count, tree_index_type)
generate_replace_ammo(count, hash_index_type)
generate_get_by_pk_ammo(count, tree_index_type)
generate_get_by_pk_ammo(count, hash_index_type)
generate_get_by_secondary_ammo(count, tree_index_type)
generate_get_by_secondary_ammo(count, hash_index_type)
